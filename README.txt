...
### Map（360°カルーセル）の並び順を手動で変える
Map画面上部に出る`location-card`（360°スポットのサムネ一覧）は、Googleスプレッドシート側の列で並び順を指定できます。

- スプレッドシート（`360`シート）に`order`（または`表示順` / `並び順` / `priority` / `sort`）列を追加
- 数値（例: 1, 2, 3...）を入れると、その昇順で並びます（未入力の行は後ろに回ります）
- 追加の列なしで並び替えたい場合は、同じカテゴリ内ではシートの行順も優先されます
